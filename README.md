...
## 📐 Unit Agent 그룹핑 기준 (Topological Criteria)

TAD 방법론에서는 다음 3가지 위상학적 기준을 바탕으로 개별 태스크를 **Unit Agent**로 통합합니다.

| 기준 | 명칭 | 핵심 질문 | 판단 근거 |
| :--- | :--- | :--- | :--- |
| **기준 1** | **Data Topology** | "같은 땅을 밟고 있는가?" | 참조하는 데이터 공간과 시맨틱 구조의 연속성 |
| **기준 2** | **Reasoning Isomorphism** | "생각의 모양이 같은가?" | 입력에서 출력까지의 논리적 처리 파이프라인 동형성 |
| **기준 3** | **Output Homotopy** | "결과의 본질이 같은가?" | 행동의 영향력 레벨 및 실행 가역성(Reversibility) |

### 🔍 상세 가이드라인

#### 1️⃣ 데이터 위상 (Data Topology)
두 태스크가 참조하는 지식 베이스나 데이터 소스의 접근 권한 및 범위가 동일해야 합니다.
* **Success Case**: 휴가 일수 조회와 사규 검색은 동일한 [HR DB] 위상 위에 있으므로 통합.
* **Fail Case**: 개인 메일함 검색과 공용 기술 문서 검색은 데이터 위상이 단절되어 있으므로 분리.

#### 2️⃣ 추론의 동형성 (Reasoning Isomorphism)
입력 데이터가 처리되어 출력으로 변환되는 '논리의 궤적'이 일치해야 합니다.
* **Success Case**: 이메일 요약과 회의록 작성은 [추출 → 재구성]이라는 동일한 위상학적 구조를 가짐.

#### 3️⃣ 출력 공간의 호모토피 (Output Homotopy)
결과물이 시스템 상태를 변화시키는 정도(Side Effect)가 같은 레벨이어야 합니다.
> ⚠️ **중요 (Hole/Singularity)**: 
> "단순 조회(Read)"와 "결제 실행(Write)" 사이에는 위상학적 단절(Hole)이 존재합니다. 이들은 **서로 다른 에이전트**로 분리하여 설계 무결성을 확보해야 합니다.
